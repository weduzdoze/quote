...
<cfset myFusebox.thisFuseaction = "loginProcess">
<cftry>
<cfoutput><cfinclude template="../model/mUser/qry_loginProcess.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 20 and right(cfcatch.MissingFileName,20) is "qry_loginProcess.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse qry_loginProcess.cfm in circuit mUser which does not exist (from fuseaction mUser.loginProcess).">
<cfelse><cfrethrow></cfif></cfcatch></cftry>
