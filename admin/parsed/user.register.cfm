...
<cfset myFusebox.thisFuseaction = "register">
<cftry>
<cfsavecontent variable="fusebox.layout"><cfoutput><cfinclude template="../view/vUser/dsp_register.cfm"></cfoutput></cfsavecontent>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 16 and right(cfcatch.MissingFileName,16) is "dsp_register.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse dsp_register.cfm in circuit vUser which does not exist (from fuseaction vUser.register).">
<cfelse><cfrethrow></cfif></cfcatch></cftry>
